		...
		for(int i = 0; i < tiles.length; i++){
			for(int j = 0; j < tiles[0].length; j++){
				if(i == posEntry[0] && j == posEntry[1] || i == posCore[0] && j == posCore[1]){
					tiles[i][j] = (TileEntity) new TileFactory(30 + (int) (40 * i), 30 + (int) (40 * j), true, debug).createEntity();
				}
				else{
					tiles[i][j] = (TileEntity) new TileFactory(30 + (int) (40 * i), 30 + (int) (40 * j), false, debug).createEntity();
				}
